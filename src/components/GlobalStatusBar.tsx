...
const RISK_TOOLTIPS: Record<string, string> = {
  CRITICAL: '무력 충돌, 제재 또는 중대한 불안정 징후 감지',
  HIGH: '높은 위협 수준 — 긴장 또는 안보 우려 지속',
  ELEVATED: '주의 위험 — 정치 불안 또는 지역 분쟁 가능성',
  LOW: '안정 — 중대한 위협 징후 없음',
};
  ...
  const tickerContent = (
    <>
      ...
      <span className="inline-flex items-center gap-1 mx-2">
        <span className="text-[#E040FB]">사이버</span>
        <span className="text-[var(--text-primary)]">{cveCount} CVE</span>
      </span>
    </>
  );
