  ...
  try {
    ...
    for (const result of lookups) {
      if (result.status === 'fulfilled') {
        ...
        results.records[type] = normalized;

        // UI compatibility: OsintPanel renders r.A/r.MX/r.NS/etc. Keep the
        // detailed `records` object, but also expose compact top-level arrays.
        if (type === 'MX') {
          results[type] = normalized.map((r: any) => {
            const [priority, ...exchange] = String(r.data).split(' ');
            return { priority: Number(priority) || 0, exchange: exchange.join(' ') || r.data };
          });
        } else if (type === 'SOA') {
          results[type] = normalized.map((r: any) => r.data);
        } else {
          results[type] = normalized.map((r: any) => r.data);
        }
      }
